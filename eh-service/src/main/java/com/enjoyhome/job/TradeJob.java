...
//import com.enjoyhome.enums.RefundStatusEnum;
//import com.enjoyhome.enums.TradingStateEnum;
//import com.enjoyhome.handler.wechat.WechatCommonPayHandler;
...
//            try {
//                //查询交易单
//                TradingVo tradingVo = this.wechatCommonPayHandler.queryTrading(BeanUtil.toBean(trading, TradingVo
//                .class));
...
//        //发送消息通知其他系统
//        // 订单 账单
//        List<Long> tradingOrderNos = tradeMsgList.stream().map(Trading::getTradingOrderNo).distinct().collect
//        (Collectors.toList());
...
//        int shardTotal = NumberUtil.max(XxlJobHelper.getShardTotal(), 1);
//
//        List<RefundRecord> list = this.refundRecordService.findListByRefundStatus(RefundStatusEnum.SENDING,
//        refundCount);
//        if (CollUtil.isEmpty(list)) {
...
//        //发送消息通知其他系统
//        // 订单 账单
//        List<Long> tradingOrderNos = tradeMsgList.stream().map(RefundRecord::getTradingOrderNo).collect(Collectors
//        .toList());
