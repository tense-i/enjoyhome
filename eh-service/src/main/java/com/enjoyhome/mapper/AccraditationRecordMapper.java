...
import com.enjoyhome.entity.AccraditationRecord;
...
public interface AccraditationRecordMapper {
    ...
    @Select("select * from accraditation_record where bussniess_id = #{bussniessId} and type = #{type} order by " +
            "step_no")
    public List<AccraditationRecord> getAccraditationRecordByBuisId(@Param("bussniessId") long bussniessId, @Param(
            "type") Integer type);

    @Select("select * from accraditation_record where bussniess_id = #{bussniessId} and type = #{type} order by " +
            "step_no desc limit 1")
